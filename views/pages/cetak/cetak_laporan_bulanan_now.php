...
class FPDF_AutoWrapTable extends FPDF
{
    ...
    public function rptDetailData()
    {
        ...
        $this->SetWidths([35, 40, 20, 20, 20, 30, 30]);
        $this->SetAligns(['L', 'L', 'R', 'R', 'R', 'R', 'R']);
        $no      = 1;
        $tawal   = 0;
        $tsin    = 0;
        $tsout   = 0;
        ...
        foreach ($this->data as $baris) {
            if ($baris['stok_akhir'] < $baris['aktual']) {
                $this->SetTextReds(['0', '0', '0', '0', '0', 255, '0']);
                $this->SetTextGreens(['0', '0', '0', '0', '0', 0, '0']);
                $this->SetTextBlues(['0', '0', '0', '0', '0', 0, '0']);
            } else {
                $this->SetTextReds(['0', '0', '0', '0', '0', 0, '0']);
                $this->SetTextGreens(['0', '0', '0', '0', '0', 0, '0']);
                $this->SetTextBlues(['0', '0', '0', '0', '0', 0, '0']);
            }
            $this->Row(
    ...
    public function Header()
    {
        ...
        if ($_GET['idsub'] == 'TQ') {
            $this->SetX($left);
            $this->Cell(0, 10, 'LAPORAN BULANAN STOCK TEST QUALITY MATERIAL', 0, 1, 'C');
        } else if ($_GET['idsub'] == 'PACKING') {
            $this->SetX($left);
            $this->Cell(0, 10, 'LAPORAN BULANAN STOCK PACKING MATERIAL', 0, 1, 'C');
        } else if ($_GET['idsub'] == 'ATK') {
            $this->SetX($left);
            $this->Cell(0, 10, 'LAPORAN BULANAN STOCK ATK MATERIAL', 0, 1, 'C');
        }
        $this->Ln(-7);
        $this->SetFont("Arial", "", 7);
        $this->SetX($left);
        $this->Cell(0, 10, 'FW-19-QCF-01 / 09', 0, 1, 'C');
        $this->settitle('Stock Barang');
        //$this->SetY(-15);
        // Line break
        ...
        $this->SetX($left += 40);
        $this->Cell(20, 10, 'Stock Awal', 1, 0, 'C');
        $this->SetX($left += 20);
        $this->Cell(20, 10, 'Kedatangan', 1, 0, 'C');
        $this->SetX($left += 20);
        $this->Cell(20, 10, 'Pengambilan', 1, 0, 'C');
        $this->SetX($left += 20);
        $this->Cell(60, 5, 'STOCK AKHIR', 1, 0, 'C');
        $this->Ln();
        $this->Cell(0, 0, '', 0, 0, 'L');
        $this->Cell(0, 0, '', 0, 0, 'L');
        $this->Cell(0, 0, '', 0, 0, 'L');
        ...
        $this->Cell(0, 0, '', 0, 0, 'L');
        $this->SetX($left += 0);
        $this->Cell(30, 5, 'Per Tanggal ' . date("d/m/Y", strtotime($_GET['tgl2'])), 1, 0, 'C');
        $this->SetX($left += 30);
        $this->Cell(30, 5, 'Per Tanggal ' . date('d/m/Y'), 1, 1, 'C');
    ...
    public function SetTextReds($a)
    {
        //Set the array of color
        $this->reds = $a;
    }

    public function SetTextGreens($a)
    {
        //Set the array of color
        $this->greens = $a;
    }

    public function SetTextBlues($a)
    {
        //Set the array of color
        $this->blues = $a;
    }
...
$data = [];
foreach ($laporanBulananNow->tampilreport($idsub, $awal, $akhir) as $rowd) {
